...
async def autopilot():
    ...
    try:
        r = await bot(
            CreateChannelRequest(
                title="𝙻𝚄𝚃𝙿𝙰𝙽 𝚄𝚂𝙴𝚁𝙱𝙾𝚃 Lᴏɢs",
                about="» Group log Created by: Lutpan Userbot\n\n» Support : @lutpansupport\n» Support: @Lutpanstein",
                megagroup=True,
            ),
        )
    ...
    if isinstance(chat.photo, ChatPhotoEmpty):
        photo = await download_file(
            "https://telegra.ph/file/ad02750e78083a8c57e90.jpg", "photoyins.jpg"
        )
...
async def autobot():
    ...
    if who.username:
        ...
        username = f"Lutpan{(str(who.id))[5:]}bot"
    bf = "@BotFather"
    ...
    if isdone.startswith("Sorry,"):
        ran = randint(1, 100)
        username = f"Lutpan{str(who.id)[6:]}{ran}bot"
        await bot.send_message(bf, username)
        await asyncio.sleep(1)
